      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
        <thead>
          <tr style={{ background: "var(--surface-2)" }}>
            <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 700, color: "var(--muted)", fontSize: 11 }}>ANNÉE</th>
            <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 700, color: "var(--muted)", fontSize: 11 }}>PÉRIODE</th>
            <th style={{ padding: "10px 14px", textAlign: "right", fontWeight: 700, color: "var(--muted)", fontSize: 11 }}>OBJECTIF</th>
            <th style={{ padding: "10px 14px", textAlign: "right", fontWeight: 700, color: "var(--muted)", fontSize: 11 }}>CA RÉALISÉ</th>
            <th style={{ padding: "10px 14px", textAlign: "center", fontWeight: 700, color: "var(--muted)", fontSize: 11 }}>PROGRESSION</th>
            <th style={{ padding: "10px 14px", textAlign: "right", fontWeight: 700, color: "var(--muted)", fontSize: 11 }}>POSTES</th>
            <th style={{ padding: "10px 14px", textAlign: "right", fontWeight: 700, color: "var(--muted)", fontSize: 11 }}>ACTIONS</th>
          </tr>
        </thead>
        <tbody>
          {fyWithCA.map(fy => {
            ...
            if (isEditing) {
              return (
                <tr key={fy.id} style={{ background: "var(--tint-amber-soft)", borderTop: "1px solid var(--line)" }}>
                  ...
                  <td style={{ padding: "10px 14px", textAlign: "right", fontWeight: 700, color: "var(--c-green)" }}>{fmtCAD(fy.ca)}</td>
            ...
            return (
              <tr key={fy.id} style={{ borderTop: "1px solid var(--line)" }}>
                <td style={{ padding: "10px 14px", fontWeight: 600, color: "var(--ink)" }}>{fy.label}</td>
                ...
                <td style={{ padding: "10px 14px", textAlign: "right", fontWeight: 600, color: "var(--muted)" }}>{fmtCAD(fy.target)}</td>
                <td style={{ padding: "10px 14px", textAlign: "right", fontWeight: 700, color: "var(--c-green)" }}>{fmtCAD(fy.ca)}</td>
                <td style={{ padding: "10px 14px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, justifyContent: "center" }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 700, color: pct >= 100 ? "var(--c-green)" : pct >= 50 ? "var(--c-blue)" : "var(--c-amber)" }}>{pct}%</span>
                  </div>
                </td>
                <td style={{ padding: "10px 14px", textAlign: "right", color: "var(--ink)" }}>{fy.count}</td>
                <td style={{ padding: "10px 14px", textAlign: "right" }}>
                  <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
