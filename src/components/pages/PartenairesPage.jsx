  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: "var(--ink)", margin: 0 }}>Partenaires externes</h2>
          <p style={{ fontSize: 13, color: "var(--muted)", margin: "4px 0 0" }}>Gérez les recruteurs partenaires et consultez les candidats proposés.</p>
        </div>
        {tab === "partners" && (
      ...
      {/* Tabs */}
      <div style={{ display: "flex", gap: 4, marginBottom: 20, background: "var(--surface-3)", borderRadius: 12, padding: 4, width: "fit-content" }}>
        <button
          ...
          style={{
            ...
            color: tab === "notifications" ? "var(--ink)" : "var(--muted)",
        ...
        >
          ...
          {newSubmissionsCount > 0 && (
            <span style={{ position: "absolute", top: -4, right: -4, background: "var(--c-red)", color: "white", fontSize: 10, fontWeight: 700, borderRadius: 10, padding: "1px 6px", minWidth: 18, textAlign: "center" }}>
              {newSubmissionsCount}
            </span>
          )}
        </button>
        <button
          ...
          style={{
            ...
            color: tab === "preselected" ? "var(--ink)" : "var(--muted)",
        ...
        <button
          ...
          style={{
            ...
            color: tab === "archived" ? "var(--ink)" : "var(--muted)",
        ...
        <button
          ...
          style={{
            ...
            color: tab === "partners" ? "var(--ink)" : "var(--muted)",
        ...
        <div>
          ...
          {currentList.length === 0 ? (
            <div className="card" style={{ textAlign: "center", padding: 40 }}>
              <div style={{ fontSize: 16, fontWeight: 600, color: "var(--ink-soft)" }}>{tab === "archived" ? "Aucun candidat archivé" : tab === "preselected" ? "Aucun candidat pré-sélectionné" : "Aucune soumission de partenaire"}</div>
              <div style={{ fontSize: 13, color: "var(--muted)", marginTop: 6 }}>{tab === "archived" ? "Les candidats archivés apparaitront ici." : tab === "preselected" ? "Les candidats pré-sélectionnés apparaitront ici." : "Les candidats proposés par vos partenaires apparaitront ici."}</div>
            </div>
          ) : filteredSubmissions.length === 0 ? (
            <div className="card" style={{ textAlign: "center", padding: 40 }}>
              <div style={{ fontSize: 14, color: "var(--muted)" }}>Aucun candidat pour ce poste.</div>
            </div>
          ) : (
                ...
                <div key={s.id} className="card" style={{ padding: "18px 20px", borderLeft: `4px solid ${borderColor(s.stage)}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 38, height: 38, borderRadius: 10, background: "var(--tint-green-soft)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 700, color: "var(--c-green)" }}>
                        {s.candidateName?.[0] || "?"}
                      </div>
                      <div>
                        <div style={{ fontSize: 15, fontWeight: 700, color: "var(--ink)" }}>{s.candidateName}</div>
                        <div style={{ fontSize: 12, color: "var(--muted)" }}>
                          Proposé par <span style={{ fontWeight: 600, color: "var(--c-green)" }}>{s.partnerName}</span>
                  ...
                  {/* Info grid */}
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 8, marginBottom: 10, padding: "10px 12px", background: "var(--surface-2)", borderRadius: 10 }}>
                  ...
                  {/* Notes / summary */}
                  {s.notes && (
                    <div style={{ padding: "10px 12px", background: "var(--tint-amber-soft)", borderRadius: 10, marginBottom: 10 }}>
                    ...
                    {s.cvFileId && (
                      <>
                        ...
                        <button
                          ...
                          style={{
                            display: "inline-flex", alignItems: "center", gap: 6,
                            padding: "7px 14px", background: "var(--tint-blue)", border: "none", borderRadius: 10,
                    ...
                    <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
                      {s.stage === "En attente" && (
                        <button
                          ...
                          style={{
                            ...
                            padding: "7px 14px", background: "var(--tint-green)", border: "none", borderRadius: 10,
                            fontSize: 12.5, fontWeight: 600, color: "var(--c-green)", cursor: "pointer", fontFamily: "inherit",
                          }}
                        >
                      ...
                      {s.stage !== "Archivé" && (
                        <button
                          ...
                          style={{
                            display: "inline-flex", alignItems: "center", gap: 5,
                            padding: "7px 14px", background: "var(--surface-3)", border: "none", borderRadius: 10,
                            fontSize: 12.5, fontWeight: 600, color: "var(--muted)", cursor: "pointer", fontFamily: "inherit",
                          }}
                        >
          ...
          <tbody>
            {filtered.map(p => (
              <tr key={p.id} style={{ borderBottom: "1px solid var(--line)" }}>
                ...
                <td style={td}>
                  <button onClick={() => openMissions(p)} style={{ background: "var(--tint-green-soft)", border: "none", borderRadius: 8, padding: "4px 10px", fontSize: 12, fontWeight: 600, color: "var(--c-green)", cursor: "pointer" }}>
                ...
                <td style={td}>
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    <button onClick={() => handleDelete(p.id)} style={{ ...actionBtn, color: "var(--c-red)" }}>Supprimer</button>
                  </div>
                </td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr><td colSpan={6} style={{ textAlign: "center", padding: 24, color: "var(--muted)" }}>Aucun partenaire trouvé</td></tr>
            )}
          </tbody>
        </table>
      </div>
        ...
        <div style={overlay}>
          <div style={modalBox}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {error && <div style={{ padding: "8px 12px", background: "var(--tint-red)", borderRadius: 10, fontSize: 12, color: "var(--c-red)" }}>{error}</div>}
        ...
        <div style={overlay}>
          <div style={{ ...modalBox, maxWidth: 600 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              ...
              <button onClick={() => setModal(null)} style={{ background: "none", border: "none", fontSize: 18, cursor: "pointer", color: "var(--muted)" }}>x</button>
            </div>

            {/* Affiliated */}
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--c-green)", textTransform: "uppercase", marginBottom: 8 }}>Missions affiliées</div>
            {affiliatedMissions.length === 0 ? (
              <div style={{ padding: 16, textAlign: "center", color: "var(--muted)", fontSize: 13 }}>Aucune mission affiliée</div>
            ) : (
              <div style={{ display: "grid", gap: 6, marginBottom: 16 }}>
                {affiliatedMissions.map(m => (
                  <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "var(--tint-green-soft)", borderRadius: 10 }}>
                    <div style={{ flex: 1, fontSize: 13 }}>
                      <span style={{ fontWeight: 600 }}>{m.title}</span>
                      <span style={{ color: "var(--muted)" }}> — {m.company}</span>
                    </div>
                    <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 6, background: m.status === "Ouverte" ? "var(--tint-green)" : "var(--surface-3)", color: m.status === "Ouverte" ? "var(--c-green)" : "var(--muted)" }}>{m.status}</span>
                    <button onClick={async () => { if (await confirmDlg("Retirer cette mission du partenaire ?", { confirmLabel: "Retirer" })) removeMission(m.id); }} style={{ background: "none", border: "none", color: "var(--c-red)", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>Retirer</button>
                  </div>
                ))}
              </div>
            )}
            ...
            {/* Add missions */}
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--c-blue)", textTransform: "uppercase", marginBottom: 8 }}>Ajouter une mission</div>
            {availableMissions.length === 0 ? (
              <div style={{ padding: 16, textAlign: "center", color: "var(--muted)", fontSize: 13 }}>Toutes les missions sont déjà affiliées</div>
            ) : (
              <div style={{ maxHeight: 240, overflow: "auto", display: "grid", gap: 4 }}>
                {availableMissions.map(m => (
                  <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "var(--surface-2)", borderRadius: 10 }}>
                    <div style={{ flex: 1, fontSize: 13 }}>
                      <span style={{ fontWeight: 600 }}>{m.title}</span>
                      <span style={{ color: "var(--muted)" }}> — {m.company}</span>
                    </div>
                    <button onClick={() => addMission(m.id)} style={{ background: "var(--tint-blue)", border: "none", borderRadius: 8, padding: "4px 12px", fontSize: 12, fontWeight: 600, color: "var(--c-blue)", cursor: "pointer" }}>
                      Affilier
                    </button>
                  </div>
                ))}
      ...
      {previewUrl && (
        <div style={overlay} onClick={closePreview}>
          <div style={{ background: "var(--surface)", borderRadius: 18, width: "90%", maxWidth: 900, height: "85vh", display: "flex", flexDirection: "column", overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 20px", borderBottom: "1px solid var(--line)" }}>
              <span style={{ fontSize: 14, fontWeight: 700, color: "var(--ink)" }}>Aperçu du CV</span>
              <button onClick={closePreview} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "var(--muted)" }}>x</button>
            </div>
        ...
        <div style={overlay}>
          <div style={{ ...modalBox, maxWidth: 500 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              ...
              <button onClick={() => setReviewModal(null)} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "var(--muted)" }}>x</button>
            </div>

            {/* Star rating */}
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: "var(--ink-2)", display: "block", marginBottom: 8 }}>Votre note</label>
              <div style={{ display: "flex", gap: 6 }}>
                {[1, 2, 3, 4, 5].map(n => (
                  <button
                    key={n}
            ...
            {/* Comment */}
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: "var(--ink-2)", display: "block", marginBottom: 6 }}>Commentaire</label>
              <textarea
            ...
            {reviews[reviewModal.candidatureId]?.length > 0 && (
              <div style={{ borderTop: "1px solid var(--line-soft)", paddingTop: 14 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase", marginBottom: 8 }}>Avis de l'équipe</div>
                  ...
                  {reviews[reviewModal.candidatureId].map(r => (
                    <div key={r.id} style={{ padding: "10px 12px", background: "var(--surface-2)", borderRadius: 10 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                        <span style={{ fontSize: 13, fontWeight: 600, color: "var(--ink)" }}>{r.userName}</span>
                      ...
                      {r.comment && <div style={{ fontSize: 12.5, color: "var(--ink-soft)", lineHeight: 1.4 }}>{r.comment}</div>}
                      <div style={{ fontSize: 10.5, color: "var(--muted)", marginTop: 4 }}>{new Date(r.updatedAt || r.createdAt).toLocaleDateString("fr-CA")}</div>
                    </div>
                  ))}
                </div>
              </div>
...
function InfoField({ label, value }) {
  return (
    <div>
      <div style={{ fontSize: 10.5, fontWeight: 600, color: "var(--muted)", textTransform: "uppercase" }}>{label}</div>
      <div style={{ fontSize: 13, color: "var(--ink)", marginTop: 1 }}>{value}</div>
    </div>
  );
}
...
function FormField({ label, value, onChange, type = "text" }) {
  return (
    <div>
      <label style={{ fontSize: 12, fontWeight: 600, color: "var(--ink-2)", display: "block", marginBottom: 4 }}>{label}</label>
      <input type={type} value={value} onChange={e => onChange(e.target.value)} className="input" style={{ width: "100%" }} />
    </div>
  );
}

const th = { textAlign: "left", padding: "10px 14px", fontSize: 11, fontWeight: 700, color: "var(--muted)", textTransform: "uppercase" };
const td = { padding: "10px 14px" };
const actionBtn = { background: "none", border: "none", fontSize: 12, fontWeight: 600, color: "var(--c-blue)", cursor: "pointer" };
const overlay = { position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 };
const modalBox = { background: "var(--surface)", borderRadius: 18, padding: 28, width: "100%", maxWidth: 460, maxHeight: "90vh", overflow: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.15)" };

function stageStyle(stage) {
  switch (stage) {
    case "En attente": return { background: "var(--tint-amber)", color: "var(--c-amber)" };
    case "Proposition partenaire": return { background: "var(--tint-green)", color: "var(--c-green)" };
    case "Soumis": return { background: "var(--tint-amber)", color: "var(--c-amber)" };
    case "Présélectionné": return { background: "var(--tint-blue)", color: "var(--c-blue)" };
    case "Archivé": return { background: "var(--surface-3)", color: "var(--muted)" };
    case "Placé": return { background: "var(--tint-blue)", color: "var(--c-blue)" };
    default: return { background: "var(--tint-violet)", color: "var(--c-indigo)" };
  }
}
