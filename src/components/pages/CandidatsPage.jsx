  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        <select className="input" aria-label="Filtrer par compétence" style={{ width: "auto", minWidth: 180 }} value={filterSkill} onChange={e => setFilterSkill(e.target.value)}>
          <option value="">Toutes les compétences</option>
          {allSkills.map(s => <option key={s} value={s}>{s}</option>)}
        </select>
        <select className="input" aria-label="Filtrer par statut de validation" style={{ width: "auto", minWidth: 200 }} value={filterValidation} onChange={e => setFilterValidation(e.target.value)}>
          <option value="">Tous les statuts validation</option>
          {validationStatuses.map(s => <option key={s.id} value={s.label}>{s.label}</option>)}
        </select>
        <select className="input" aria-label="Filtrer par propriétaire" style={{ width: "auto", minWidth: 180 }} value={filterOwner} onChange={e => setFilterOwner(e.target.value)}>
      ...
      {detail && (
        <div className="modal-bg" onClick={e => e.target === e.currentTarget && setDetailId(null)}>
          <FicheCandidat contact={detail} onClose={() => setDetailId(null)} onEdit={() => { onEdit(detail); setDetailId(null); }} onDelete={() => onDelete(detail.id)} onAddCandidature={onAddCandidature} goToMission={goToMission ? (id) => { setDetailId(null); goToMission(id); } : undefined} candidatures={candidatures} missions={missions} loadAll={loadAll} validationStatuses={validationStatuses} />
        </div>
      )}
    </div>
  );
