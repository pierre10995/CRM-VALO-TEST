    ...
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <button onClick={() => onSelect("all")} className="btn" style={{
          ...
          color: selectedFY === "all" ? "white" : "var(--muted)",
          border: selectedFY === "all" ? "none" : "1.5px solid var(--line)",
        }}>Toutes</button>
        {fiscalYears.map(fy => {
          ...
          return (
            <button key={fy.id} onClick={() => onSelect(String(fy.id))} className="btn" style={{
              ...
              color: selectedFY === String(fy.id) ? "white" : "var(--muted)",
              border: selectedFY === String(fy.id) ? "none" : "1.5px solid var(--line)",
