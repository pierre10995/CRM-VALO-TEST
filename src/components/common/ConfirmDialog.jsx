          ...
          <div className="card" role="alertdialog" aria-modal="true" aria-label={dialog.title} aria-describedby="confirm-dialog-message" style={{ width: "min(420px, 100%)", padding: 24 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
              <div style={{
                ...
                background: dialog.danger ? "var(--tint-red)" : "var(--tint-blue)",
                display: "flex", alignItems: "center", justifyContent: "center",
              }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={dialog.danger ? "var(--c-red)" : "var(--c-blue)"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M12 9v4m0 4h.01M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" />
                </svg>
              </div>
              <h2 style={{ fontSize: 16, fontWeight: 700, color: "var(--ink)" }}>{dialog.title}</h2>
            </div>
            <p id="confirm-dialog-message" style={{ fontSize: 13.5, color: "var(--ink-soft)", lineHeight: 1.5, marginBottom: 20 }}>{dialog.message}</p>
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              ...
              <button
                ...
                style={dialog.danger ? { background: "var(--c-red)", color: "white" } : undefined}
                onClick={() => close(true)}
              >
                {dialog.confirmLabel}
              </button>
