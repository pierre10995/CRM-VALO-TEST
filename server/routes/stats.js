...
router.get("/validation-statuses", async (req, res) => {
  try {
...
router.post("/validation-statuses", async (req, res) => {
...
router.put("/validation-statuses/:id", async (req, res) => {
  const { label, bg, color } = req.body;
  if (!label) return res.status(400).json({ error: "Libellé requis" });
  try {
    const { rows } = await pool.query(
...
// CV Summary
router.post("/cv-summary/generate", async (req, res) => {
